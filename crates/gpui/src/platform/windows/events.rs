...
impl WindowsWindowInner {
    ...
    fn handle_mouse_leave_msg(&self, handle: HWND) -> Option<isize> {
        // The pointer can leave with no final WM_MOUSEMOVE outside the element it
        // was over, so report the exit, as the other platforms do, for hover state
        // to clear. Crossing into this window's own frame is no exit: the
        // WM_NCMOUSEMOVE that follows keeps the window hovered.
