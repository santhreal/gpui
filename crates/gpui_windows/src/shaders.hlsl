...
ShadowVertexOutput shadow_vertex(uint vertex_id: SV_VertexID, uint instance_id: SV_InstanceID) {
    ...
    float4 device_position = to_device_position_transformed(unit_vertex, bounds, shadow.transformation);
    float4 clip_distance = distance_from_clip_rect_transformed(unit_vertex, bounds, shadow.content_mask, shadow.transformation);
...
PathVertexOutput path_rasterization_vertex(uint vertex_id: SV_VertexID) {
    ...
    float2 transformed = mul(sprite.xy_position, sprite.transformation.rotation_scale) + sprite.transformation.translation;
...
UnderlineVertexOutput underline_vertex(uint vertex_id: SV_VertexID, uint instance_id: SV_InstanceID) {
    ...
    float4 device_position = to_device_position_transformed(unit_vertex, underline.bounds, underline.transformation);
